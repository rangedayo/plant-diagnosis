...
그래서 방법을 바꿨습니다. 앞 단계의 정보를 받아 AI가 추론을 마친 다음에, 그 추론을 검사해서 과한 진단을 코드로 교정하는 단계를 붙였습니다. 잎 끝이 살짝 변색된 정도면 건강으로 내리고, 진짜 병처럼 보이는 단어가 있으면 절대 손대지 않고 AI의 추론을 그대로 통과시킵니다. 그 결과 **아픈 걸 놓치는 실수(FN)는 0건을 유지하면서 FP(건강한데 아프다고 한 경우)를 절반으로 줄였습니다.**
...
2. **키워드를 정리한다** — 그 증상을 검색에 쓸 키워드로 정리합니다.
3. **자료를 찾는다** — 키워드로 식물 자료(벡터 DB)에서 비슷한 내용을 검색합니다.
4. **진단을 쓴다** — 찾은 자료를 바탕으로 진단 문장을 만듭니다.
5. **검사하고 고친다** — 과한 진단을 코드로 교정하는 안전 게이트 단계입니다.
