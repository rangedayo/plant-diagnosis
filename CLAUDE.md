...
- **🏁 R17 (generate 경미 출력) = 정량 평가 종료점**: `eval/after_acc_r17_generate_mild.json` — generate 프롬프트에 경미 status·판정 규칙 추가(가드 무변경). R16 baseline 대비 측정.
  - **3×3 tier 혼동표** (행=gt_tier, 열=pred_tier; scored 39, skipped 0):
    | gt\pred | 건강 | 경미 | 비건강 |
    |---|---|---|---|
    | **건강**(16) | 11 | 3 | 2 |
...
### 4.2 cardinal_miss 0 절대 사수 (Recall 게이트, 3단 렌즈)

**recall 게이트 = `cardinal_miss = 0`** (gt비건강→pred건강 = 0). 이게 **절대 사수 하드 게이트**.

- 어떤 변경이든 cardinal_miss 1+ 발생 시 즉시 revert 검토.
- `soft_miss`(gt비건강→pred경미)는 **추적·최소화 대상이되 하드 게이트 아님** — 완전 실패가 아니라 "한 칸 과소"(감점).
- **이진 `post_guard.fn`/recall은 R17부터 보조·역사 지표로 강등**: 경미가 is_healthy=True로 묶이며 비건강 GT의 pred경미가 이진상 FN으로 새는데, 이는 3단 렌즈에선 soft_miss이지 cardinal 실패가 아니다. 이진 FN 0은 더 이상 사수 대상이 아님(역사적 R16까지의 기준).
- "cardinal_miss가 견고한가" 항상 점검 (analyze 비결정성에 의존하는 0은 견고하지 않음).
...
- **시도**: 정성 평가 Finding 2 대응으로 "불확실 식별 시 출력 종명을 억제(헤징)"하는 트리거(`_species_identification_uncertain`)+프롬프트 구현(미푸시 `e642ede`).
- **문제**: 올바른 의도는 종명을 **보여주되** 관찰 근거 기반 **추정 톤**으로 쓰는 것(확신 단정만 금지). 억제는 방향 자체가 오해.
- **대책**: 미푸시였으므로 `git reset --hard origin/main`으로 히스토리 더럽힘 없이 제거 → 증거-톤 프롬프트(`[설명 톤/프레이밍]`)로 대체. 트리거/조건 분기 없이 전 케이스 동일 톤.
- **교훈**: 출력 표면 조정 전에 "억제 vs 톤" 같은 의도 방향을 먼저 사용자와 합의. 미푸시 로컬은 revert보다 reset이 깨끗.
...
- **비대칭 게이트 원칙(3단 채점 라운드 설계 기준)**: 비건강→건강 오분류(`cardinal_miss`) = **0 (하드 게이트, recall 사수)**, 비건강→경미(`soft_miss`) = **추적·최소화 대상이되 하드 게이트 아님**(완전 실패는 아니나 감점), 과대(건강/경미→비건강) = **최소화**. **recall 게이트 = cardinal_miss 0**이며, 이진 FN/recall은 R17부터 보조·역사 지표로 강등(경미↔is_healthy=True 매핑 탓에 soft_miss가 이진 FN으로 새므로 이진 FN 0은 더 이상 사수 대상 아님 — §4.2).
...
- **2차 보정(refine) 평가 하니스 부재**: 1차 진단은 `run_eval.py`로 측정되나, 챗봇 2차 보정(`/diagnose/refine`, generate-only) 출력은 **측정 경로 없음**(구조적 공백). 객관식 답변이 처방을 더 정확히 만드는지 정량 검증하려면 2차 전용 하니스(질문-답변 픽스처 + 1차→2차 변화 채점) 신설 필요. 현재는 수동 E2E·LLM sanity뿐.
- **최근 진단 cross-plant 조회 = fan-out 채택**: 홈 "최근 진단 기록"은 `lib/db.listRecentDiagnoses`가 식물별 최신 N건을 모아 병합·정렬(스키마/보안규칙 무변경, YAGNI). 식물 수가 많아지면 `collectionGroup("diagnoses")` + 각 문서 `uid` 필드 + 복합 인덱스 + 보안규칙 + 기존 문서 마이그레이션으로 전환 여지(현 단계 불필요).

> **앱 구현 단계 종료(2026-06-11)**: 진단 파이프라인 + 제품 UI(4뷰·챗봇 2차 보정·홈/타임라인/비교) 구현 완료. **다음 품질 트랙 = 평가셋 확장**(아래 항목 묶음, 비시급): 과습·영양 부족 독립 GT 표본 확보, 경미 tier 표본 확충, plantvillage_50 tier 재생성 등으로 5-status·3단 혼동표 완성 → 측정 라운드 재개. 평가셋이 충분해지기 전까지는 신규 프롬프트/가드 측정 라운드 보류.
...
6. ☐ Atomic 커밋 분리 계획
7. ☐ 푸시 보류 (사용자 검토 후)
8. ☐ cardinal_miss=0 사수 게이트 의식 (이진 FN/recall은 R17부터 보조·역사 지표)
...
*마지막 업데이트: 2026-06-11 — 제품 단계 2차 완료 + 앱 구현 종료. 챗봇 2차 보정(generate-only `/diagnose/refine`, 백엔드 + 프론트 단계2·3: 진입 카드·질문 전용 화면·변화 배너·1차/2차 토글, echo-back 무결성·cardinal_miss 0 구조 보존) + 홈 화면 묶음(진단 탭 제거 3탭·홈 아이콘 stroke화·업로드 미리보기+진단 시작 이연·최근 진단 cross-plant fan-out + 큰 사진 카드 2개) 푸시 완료(~8b688ac). 진단 로직·게이트·앵커(§2 R16/R17) 무변경 — 전부 프론트/제품 변경이라 측정 없음.*
*다음 품질 트랙: 평가셋 확장(§9 — 과습·영양 부족·경미 tier 표본 확충 → 5-status·3단 혼동표 완성)이 선행 과제. 비시급. 평가셋이 충분해지면 측정 라운드 재개.*
*다음 업데이트 트리거: 평가셋 확장 착수 또는 측정 라운드 재개 시(이때 §2 앵커·§6 보고 규율 다시 활성).*
