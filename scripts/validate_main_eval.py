"""main_eval 평가셋 라벨 전체 검증 진입점.

설계: docs/design/design_accuracy_track.md (ACC-R2)
성격: 평가셋 라벨 검증 (진단/프론트 무관, LLM/Vision 호출 없음).

동작:
    - test_data/main_eval/labels.json 을 utf-8-sig 로 로드.
    - labeling_vocab.validate_dataset 로 전체 순회 → 분포 리포트 출력.
    - 종료 코드 = 위반(violation) 수 (0 = 전부 통과).

검증 항목 (validate_label에 위임):
    - ground_truth 필수 필드 누락
    - symptoms enum 위반 / unhealthy인데 symptoms 없음
    - true_status enum 위반 (TODO 등 미기입 포함)
    - is_healthy ↔ true_status 정합성 위반

실행 (프로젝트 루트에서)::

    python scripts/validate_main_eval.py
"""
