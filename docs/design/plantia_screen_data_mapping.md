...
### 이렇게 판단했어요 카드 (구 "원인 설명")

| UI 영역 | 필드 경로 | 타입 |
|---------|----------|------|
| 원인 본문 | `structured_result.cause` | `str` |

> 카드 제목은 **"이렇게 판단했어요"** (R6). 건강/병해 상태 무관하게 자연스럽도록 "원인"이라는 표현을 뺌. 본문(`cause`)·구조는 불변.
...
> R6: 가로 4컬럼 그리드 → **세로 1열 행 스택**(아이콘+라벨+값, 환경 정보 행과 통일). 실데이터 텍스트(특히 `placement`)가 길어 좁은 열에서 줄바꿈이 지저분해지던 문제 해소. 정본 시안(`plantia_care_guide.html`·`plantia_combined.html`)도 동일 레이아웃으로 갱신됨.
